#!/bin/bash
# JSCeudo
# PSSO Enforcement Helper Script made for Jamf Setup Checklist
# Based primarily on https://github.com/Macjutsu/pseudo by Kevin M. White
...
# Check to see if the current user is registered for Platform SSO and set "${psso_user_status_dscl}", "${psso_user_status_login_name}", and "${psso_user_status_state} accordingly.
check_psso_user_status() {
	...
	if [[ -n "${dscl_result}" ]]; then
		...
		psso_user_status_login_name=$(echo "${app_sso_response}" | sed -e '1,/User Configuration:/d' | jq -r '.userLoginConfiguration.loginUserName' 2> /dev/null)
		psso_user_status_state=$(echo "${app_sso_response}" | sed -e '1,/User Configuration:/d' | jq -r '.state' 2> /dev/null)
	fi
	[[ -z "${dscl_result}" ]] && psso_user_status_dscl="FALSE"
	[[ -z "${psso_user_status_login_name}" ]] && psso_user_status_login_name="FALSE"
	[[ ! "${psso_user_status_state}" == "POUserStateNormal (0)" ]] && psso_user_status_state="FALSE"
}
...
# Open the Platform SSO registration window.
...
tell application "System Events"
	set visibleApps to every process whose visible is true and name is not "AppSSOAgent" and name is not "Jamf Conditional Access" and name is not "Single Sign-On" and name is not "Setup Checklist" and name is not "Finder"
	repeat with anApp in visibleApps
		tell anApp
			set visible to false
		end tell
...
workflow_psso() {
	...
	while [[ "${psso_user_status_dscl}" == "FALSE" ]] || [[ "${psso_user_status_state}" == "FALSE" ]]; do
		if [[ "$(check_psso_registration_status)" == "FALSE" ]]; then
			log_pseudo "Status: Attempting to open Platform SSO registration..."
			open_psso_registration
			[[ "$(check_psso_registration_status)" == "FALSE" ]] && log_pseudo "Error: Unable to open Platform SSO registration."
		fi
	...
	if [[ "${psso_workflow_active}" == "TRUE" ]]; then
		while [[ ! "$(check_psso_registration_active)" == "FALSE" ]]; do
			sleep 0.1
		done
		log_pseudo "Status: Platform SSO is now registered for local user ${current_user_account_name} to account ${psso_user_status_login_name}."
	else
